...
# encoding: utf-8
"""
DB2/ValueOfSerializability/run.py
...
TIMING_FILE_PATH  = '.'   # Path of the timings file timing.txt (overwritten)
OUTPUT_FILE_PATH  = './output.txt'   # Path of the output file output.txt (append)
...
def swap(q):
    #start = time.time()
    #sys.stderr.write("Starting swap %s\n"%start)
...
def summation(q):
    ...
    for n in range(int(round(NBSWAPS/NBSWAPTHREADS))):
        ...
        status = ibm_db.close(conn)
        if status == False: raise Usage("Failed to close db connection.\n")
        if n<NBSUMRUNS-1: time.sleep(SUMSLEEP*NBSWAPS/NBSWAPTHREADS)
        #sys.stderr.write("end sum %s. Did %d sums\n"%((time.time()-start),NBSWAPS))
...
def experiment(q):
    ...
    start = time.time()
    for n in range(NBSWAPTHREADS):
        sthreads.append(Thread(summation, q))

    for t in sthreads:
        t.join()
    stop = time.time()
    timing = stop-start
    print "%f"%(NBSWAPS/timing)
    # Barrier
    ...
    sys.stderr.write("=====End experiment run=====\n")
...
Example: python run.py -t10 -s1000 -r5 -iCS
'''
...
def main(argv=None):
    ...
    global TIMING_FILE_PATH, OUTPUT_FILE_PATH
    global q
    ...
    try:
        try:
            opts, args = getopt.getopt(argv[1:], 
                                       "ho:vr:s:t:g:i:c:", 
            ["help", "output=", "runs=","swaps=", "threads=",
             "isol=","sumcount="])
        ...
        for option, value in opts:
            ...
            if option in ("-c","--sumcount"):
                v = int(value)
                if (v < 0 or v>10): raise Usage("Sumcount out of bounds")
                NBSUMRUNS=v
